...
//커스텀 에디터
[CustomEditor(typeof(FemaleTPPrefabMaker))]


public class EditorFemalePrefabMaker : Editor
{
    public override void OnInspectorGUI()
    {
        FemaleTPPrefabMaker myPrefabMaker = (FemaleTPPrefabMaker)target;

        //allOptions이 false라면 -> 시작할 때 alloptions은 false이다.
        if (!myPrefabMaker.allOptions)
        {
            //LET'S GET DRESS 버튼 생성!
            if (GUILayout.Button("LET'S GET DRESS", GUILayout.Width(250), GUILayout.Height(75)))
            {
                // Menu / Getredy 함수 실행시키기
                myPrefabMaker.Menu();
        ...
        else
        {
            ...
            //------------------------------------------------------------------
            // Inspector 에 한줄에 2개의 버튼을 생성하는 함수
            // 테스트
            GUILayout.BeginVertical("box");

            EditorGUILayout.LabelField("------------Test------------", GUILayout.Width(200), GUILayout.Height(20));

            GUILayout.BeginHorizontal("box");

            if (GUILayout.Button("Test1 출력", GUILayout.Width(100), GUILayout.Height(50)))
            {
                myPrefabMaker.Print_Test(4);
            }
            if (GUILayout.Button("Test2 출력", GUILayout.Width(100), GUILayout.Height(50)))
            {
                myPrefabMaker.Print_Test(4);
            }
            GUILayout.EndHorizontal();

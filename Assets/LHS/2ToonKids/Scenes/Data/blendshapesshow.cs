...
namespace ToonKids
{
    public class blendshapesshow : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            for (int forAUX = 0; forAUX < Heads.Length; forAUX++)
            {
                if (forAUX < (Heads.Length / 2)) GOAUX[forAUX].transform.position = transform.position ;
                if (forAUX > (Heads.Length / 2) - 1) GOAUX[forAUX].transform.position = transform.position  + (transform.up * -0.35f);
            }
        }
